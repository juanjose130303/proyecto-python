...
class Gestion:
    ...
    @staticmethod
    def inscribir_entrenador(entrenadores, rutas):  
        print("Inscripción de nuevo Entrenador:")
        nombre = input("Nombre: ")
        apellido = input("Apellido: ")
        ruta = input("Ruta de entrenamiento (ID): ")
        celular = input("Número de celular: ")
        direccion = input("Dirección: ")

       
        ruta_seleccionada = buscar_ruta_por_id(rutas, ruta)
        if ruta_seleccionada:
            
            nuevo_entrenador = Entrenador(
                id_entrenador=len(entrenadores) + 1,  
                nombre=nombre,
                apellido=apellido,
                horario_entrenador='activo',  
                ruta_entrenamiento=ruta_seleccionada,
                nro_celular=celular,
                direccion=direccion
            )
            entrenadores.append(nuevo_entrenador)
            print("Entrenador inscrito exitosamente.")
        else:
            print("Ruta no encontrada.")
            
    def buscar_ruta_por_id(rutas, id_ruta):
        for ruta in rutas:
            if str(ruta.id_ruta) == id_ruta:
                return ruta
        return None
